...
import ApiActionConfig from "../common/api-action-config";
import { InlineApiActionDto, TaskApiAction } from "@/app/_types/action.types";
...
export default function CreateTaskModal({ open, onOpenChange, onTaskCreated, task }: CreateTaskModalProps) {
    ...
    const [apiActions, setApiActions] = useState<InlineApiActionDto[]>([]);
    ...
    useEffect(() => {
        ...
        if (open) {
            ...
            if (task) {
                ...
                // Convert existing TaskApiAction to InlineApiActionDto for editing
                setApiActions(
                    (task.apiActions || []).map((action: TaskApiAction) => ({
